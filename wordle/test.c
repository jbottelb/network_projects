...
int main() {
    ...
    word = "board";
    if (in_word_list(word, "server/word_list.txt") == 0){
        printf("Word in list\n");
    } else {
    ...
    char *mword = select_word(w, "server/word_list.txt");
    ...
    while (strstr("yes", me->winner) == 0 && 1 == 0){
        ...
        char my_guess[5];
        scanf("%s", my_guess);
        if (in_word_list(my_guess, "server/word_list.txt") == 1){
    ...
    pls[1] = p2;
    printf("%s\n", pls[1]->name);
    Player *found = find_player(pls, "Josh", 2);
    if (found){
